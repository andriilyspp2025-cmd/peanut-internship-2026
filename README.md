...
Our execution and accounting pipeline orchestrates opportunities across both centralized and decentralized venues. The \ArbChecker\ serves as the integration layer:
...
graph TD
    ...
    B --> B1[AMM Math & Mempool Monitor]
    C --> C1[OrderBookAnalyzer]
...
- **Strict Inventory Execution Guards**: Opportunities are only executed if strict pre-flight checks (\can_execute\) pass against the multi-venue \InventoryTracker\. The \RebalancePlanner\ detects asset skews proactively.
...
We provide a built-in Makefile target to run the end-to-end integration demo showing the calculated gap, fees, and inventory checks.
